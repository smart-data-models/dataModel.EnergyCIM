...
グローバルな記述です。**CIMのデータモデルからの採用です。このクラスはIEEE Std 421.5-2005のタイプST5Bモデルを表しています。タイプST5Bの励磁システムは、タイプST1Aモデルのバリエーションであり、代替の過励磁および不足励磁の入力と追加の制限があります。  参考にしてください。IEEE規格421.5-2005の7.5項。   注：IEEE 421.5規格のブロック図では、入力信号がVcであり、Vrefとの和算点が示されていません。ExcIEEEST5Bの実装では、Vrefとの和算点を考慮する必要があります。  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `kc`: 整流器のレギュレーションファクター（K）。  代表値＝0.004。初期値：0.0  - `kr`: レギュレータゲイン（K）。  代表値＝200。初期値：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `t1`: 発射回路時定数（T1）。  代表値＝0.004。デフォルト：0  - `tb1`: レギュレータ・ラグ時定数（T）。  代表値＝6。初期値：0  - `tb2`: レギュレータ・ラグ時定数（T）。  代表値＝0.01。デフォルト：0  - `tc1`: レギュレータリード時定数（T）。  代表値＝0.8です。デフォルト：0  - `tc2`: レギュレータリード時定数（T）。  代表値＝0.08。デフォルト：0  - `tob1`: OELのラグ時定数（T）。  代表値＝2。デフォルト：0  - `tob2`: OELのラグ時定数（T）。  代表値＝0.08。デフォルト：0  - `toc1`: OELのリード時定数（T）。  代表値＝0.1です。デフォルト：0  - `toc2`: OELのリード時定数（T）。  代表値＝0.08。デフォルト：0  - `tub1`: UELのラグ時定数（T）。  代表値＝10。デフォルト：0  - `tub2`: UELのラグ時定数（T）。  代表値＝0.05。デフォルト：0  - `tuc1`: UELのリード時定数（T）。  代表値＝2。デフォルト：0  - `tuc2`: UELのリード時定数（T）。  代表値＝0.1。デフォルト：0  - `type`: NGSIタイプ。ExcIEEEST5Bである必要があります。  - `vrmax`: ボルテージレギュレータの最大出力（V）。  代表値＝5。初期値：0.0  - `vrmin`: ボルテージレギュレーターの最低出力（V）。  代表値＝-4。初期値：0.0    
...
JSON-LD形式のExcIEEEST5Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST5Bの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcIEEEST5Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST5Bの例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
