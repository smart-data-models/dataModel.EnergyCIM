...
グローバルな記述です。**CIMデータモデルから採用。トランス給電による静的励磁システム（ABBレギュレータによる静的）。このモデルは、主発電機端子の変圧器によって給電されるゲート付きサイリスタブリッジが主発電機に直接給電する静的励磁システムを表している。  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `efdmax`: 最大開放電圧（Efdmax）。  代表値＝5。初期値：0.0  - `efdmin`: 最小開放電圧（Efdmin）。  代表値＝-5。初期値：0.0  - `id`: エンティティのユニークな識別子  - `k`: 定常時のゲイン（K）。  代表値＝300。デフォルト：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `switch`: 補助信号のルーティングセレクタ（スイッチ）。true = Vsが3番目のサミングポイントに接続されている false = Vsが1番目のサミングポイントに接続されている（図参照）。代表値＝true。デフォルトは偽  - `t1`: コントローラー時定数（T1）。  代表値＝6。デフォルト：0  - `t2`: コントローラー時定数（T2）。  代表値＝1。デフォルト：0  - `t3`: リード／ラグ時定数（T3）。  代表値＝0.05。デフォルト：0  - `t4`: リード／ラグ時定数（T4）。  代表値＝0.01。デフォルト：0  - `type`: NGSIタイプです。ExcBBCでなければならない  - `vrmax`: 制御素子の最大出力（Vrmax）。  代表値＝5。初期値：0.0  - `vrmin`: 制御素子の最小出力（Vrmin）。  代表値＝-5。初期値：0.0  - `xe`: 有効な励磁トランスのリアクタンス（Xe）。  代表値＝0.05。初期値：0.0    
...
JSON-LD形式のExcBBCの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
ExcBBCをJSON-LD形式で正規化した例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcBBCの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
ExcBBCをJSON-LD形式で正規化した例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
