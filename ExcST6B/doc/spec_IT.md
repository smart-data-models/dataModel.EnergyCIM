...
Descrizione globale: **Adattato dai modelli di dati CIM. Sistema di eccitazione statica IEEE ST6B modificato con controllore PID e anello di feedback interno opzionale.**  

## Elenco delle proprietà  

- `address`: L'indirizzo postale  - `alternateName`: Un nome alternativo per questa voce  - `areaServed`: L'area geografica in cui viene fornito un servizio o un articolo offerto  - `dataProvider`: Una sequenza di caratteri che identifica il fornitore dell'entità di dati armonizzata.  - `dateCreated`: Timestamp di creazione dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `dateModified`: Timestamp dell'ultima modifica dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `description`: Una descrizione di questo articolo  - `id`: Identificatore unico dell'entità  - `ilr`: Riferimento del limite di corrente di uscita dell'eccitatore (Ilr).  Valore tipico = 4.164. Predefinito: 0.0  - `k1`: Selettore (K1). true = il feedback è da Ifd false = il feedback non è da Ifd. Valore tipico = true. Valore predefinito: Falso  - `kcl`: Regolazione del limite della corrente di uscita dell'eccitatore (Kcl).  Valore tipico = 1.0577. Predefinito: 0,0  - `kff`: Costante di guadagno di precontrollo del regolatore di campo ad anello interno (Kff).  Valore tipico = 1. Predefinito: 0,0  - `kg`: Costante di guadagno di feedback del regolatore di campo ad anello interno (Kg).  Valore tipico = 1. Predefinito: 0.0  - `kia`: Guadagno integrale del regolatore di tensione (Kia).  Valore tipico = 45.094. Predefinito: 0.0  - `klr`: Regolazione del limite della corrente di uscita dell'eccitatore (Kcl).  Valore tipico = 17.33. Predefinito: 0.0  - `km`: Costante di guadagno in avanti del regolatore di campo ad anello interno (Km).  Valore tipico = 1. Predefinito: 0,0  - `kpa`: Guadagno proporzionale del regolatore di tensione (Kpa).  Valore tipico = 18,038. Predefinito: 0.0  - `kvd`: Guadagno derivato del regolatore di tensione (Kvd).  Valore tipico = 0. Predefinito: 0.0  - `location`: Riferimento Geojson all'elemento. Può essere Point, LineString, Polygon, MultiPoint, MultiLineString o MultiPolygon  - `name`: Il nome di questo articolo.  - `oelin`: Selettore ingresso OEL (OELin). Valore tipico = noOELinput. Valore predefinito: Nessuno  - `owner`: Una lista contenente una sequenza di caratteri codificata in JSON che si riferisce agli ID unici dei proprietari  - `seeAlso`: elenco di uri che puntano a risorse aggiuntive sull'elemento  - `source`: Una sequenza di caratteri che dà la fonte originale dei dati dell'entità come URL. Si raccomanda di essere il nome di dominio completamente qualificato del fornitore di origine, o l'URL dell'oggetto di origine.  - `tg`: Costante di tempo di feedback del regolatore di tensione di campo ad anello interno (Tg).  Valore tipico = 0,02. Predefinito: 0  - `ts`: Costante di tempo di accensione del raddrizzatore (Ts).  Valore tipico = 0. Predefinito: 0  - `tvd`: Guadagno derivato del regolatore di tensione (Tvd).  Valore tipico = 0. Predefinito: 0  - `type`: Tipo NGSI. Deve essere ExcST6B  - `vamax`: Uscita massima del regolatore di tensione (Vamax).  Valore tipico = 4,81. Predefinito: 0.0  - `vamin`: Uscita minima del regolatore di tensione (Vamin).  Valore tipico = -3.85. Predefinito: 0.0  - `vilim`: Selettore (Vilim). true = il limitatore Vimin-Vimax è attivo false = il limitatore Vimin-Vimax non è attivo. Valore tipico = true. Valore predefinito: Falso  - `vimax`: Limite massimo di ingresso del regolatore di tensione (Vimax).  Valore tipico = 10. Predefinito: 0.0  - `vimin`: Limite minimo di ingresso del regolatore di tensione (Vimin).  Valore tipico = -10. Predefinito: 0.0  - `vmult`: Selettore (Vmult). true = moltiplicare l'uscita del regolatore per la tensione del terminale false = non moltiplicare l'uscita del regolatore per la tensione del terminale.  Valore tipico = true. Valore predefinito: Falso  - `vrmax`: Uscita massima del regolatore di tensione (Vrmax).  Valore tipico = 4,81. Predefinito: 0.0  - `vrmin`: Uscita minima del regolatore di tensione (Vrmin).  Valore tipico = -3.85. Predefinito: 0.0  - `xc`: Reattanza della sorgente di eccitazione (Xc).  Valore tipico = 0,05. Predefinito: 0.0    
...
Non disponibile l'esempio di un ExcST6B in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-v2 quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
...
Non disponibile l'esempio di un ExcST6B in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-LD quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
