...
グローバルな記述です。**CIMデータモデルから採用。固定子電流リミッタ(SCL)および電流補償器(DROOP)を入力していないIEEE ST7B静止励磁システムを修正したもの。  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `kh`: 価値の高いゲートフィードバックゲイン（Kh）。  典型的な値＝1です。初期値：0.0  - `kia`: 電圧調整器の積分利得（Kia）。  代表値＝1。初期値：0.0  - `kl`: 低値のゲートフィードバックゲイン（Kl）。  典型的な値＝1です。初期値：0.0  - `kpa`: 電圧調整器の比例ゲイン（Kpa）。  代表値＝40。デフォルト：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `oelin`: OEL入力セレクタ（OELin）。代表値＝noOELinput。デフォルトなし  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `tb`: レギュレータ・ラグ時定数（Tb）。  代表値＝1。デフォルト：0  - `tc`: レギュレータリード時定数（Tc）。  代表値＝1。デフォルト：0  - `tf`: 励磁制御系安定化時定数（Tf）。  代表値＝1。デフォルト：0  - `tg`: 内側ループのフィールド電圧調整器のフィードバック時定数（Tg）。  代表値＝1。初期値：0  - `tia`: フィードバック時定数（Tia）。  代表値＝3デフォルト：0  - `ts`: Rectifier firing time constant (Ts)。  代表値＝0、デフォルト：0  - `type`: NGSIタイプです。ExcST7Bである必要があります。  - `uelin`: UEL入力セレクタ（UELin）。代表値＝noUELinput。デフォルトなし  - `vmax`: 最大電圧基準信号（Vmax）。  代表値＝1.1。初期値：0.0  - `vmin`: 最小電圧基準信号（Vmin）。  代表値＝0.9初期値：0.0  - `vrmax`: ボルテージレギュレータの最大出力（Vrmax）。  代表値＝5。初期値：0.0  - `vrmin`: ボルテージレギュレーターの最小出力（Vrmin）。  代表値＝-4.5。初期値：0.0    
...
JSON-LD形式のExcST7Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcST7Bの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
ExcST7Bの例をkey-valuesとしてJSON-LD形式にしたものは利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcST7Bの例を利用できません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
