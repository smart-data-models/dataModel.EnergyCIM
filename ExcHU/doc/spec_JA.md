...
グローバルな記述です。**CIMデータモデルから採用。電圧変換器を内蔵したハンガリーの励磁システムモデル。  

## プロパティのリスト  

- `address`: 郵送先住所  - `ae`: メジャーループPIタグのゲインファクター（Ae）。  代表値＝3。初期値：0.0  - `ai`: マイナーループPIタグのゲインファクター（Ai）。  代表値＝22。デフォルト：0.0  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `atr`: AVR定数（Atr）。  代表値＝2.19です。初期値：0.0  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `emax`: AVRベースのフィールド電圧制御信号の上限値（Emax）。  代表値＝0.996です。初期値：0.0  - `emin`: AVRベースのフィールド電圧制御信号の下限値（Emin）。  代表値＝-0.866 既定値：0.0  - `id`: エンティティのユニークな識別子  - `imax`: メジャーループPIタグの出力信号の上限値（Imax）。  代表値＝2.19。初期値：0.0  - `imin`: メジャーループPIタグの出力信号の下限値（Imin）。  代表値＝0.1です。初期値：0.0  - `ke`: 電圧ベースの変換定数（Ke）。  代表値＝4.666。初期値：0.0  - `ki`: 電流ベースの変換定数（Ki）。  代表値＝0.21428。初期値：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `te`: メジャーループPIタグの統合時定数（Te）。  代表値＝0.154。デフォルト：0  - `ti`: マイナーループPI制御タグの積分時定数（Ti）。  代表値＝0.01333。初期値：0  - `tr`: フィルター時定数（Tr）。この励磁システムモデルに電圧補償器を併用する場合、Trは0に設定する必要があります。 典型的な値は0.01です。デフォルト：0  - `type`: NGSIタイプです。ExcHUでなければならない    
...
JSON-LD形式のExcHUの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcHUの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcHUの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcHUの例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
