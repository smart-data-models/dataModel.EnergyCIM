...
Description globale : **Adapté des modèles de données CIM. Modèle slovaque du système d'excitation.  Le limiteur de sous-excitation et le contrôle de la tension secondaire sont inclus dans ce modèle. Lorsque ce modèle est utilisé, il ne peut pas y avoir de modèle séparé de limiteur de sous-excitation ou de contrôleur VAr**.  
