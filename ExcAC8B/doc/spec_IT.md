...
Descrizione globale: **Adattato dai modelli di dati CIM. Sistema di eccitazione del raddrizzatore IEEE AC8B modificato alimentato dall'alternatore con ingresso di velocità e limitatore di ingresso.**  

## Elenco delle proprietà  

- `address`: L'indirizzo postale  - `alternateName`: Un nome alternativo per questa voce  - `areaServed`: L'area geografica in cui viene fornito un servizio o un articolo offerto  - `dataProvider`: Una sequenza di caratteri che identifica il fornitore dell'entità di dati armonizzata.  - `dateCreated`: Timestamp di creazione dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `dateModified`: Timestamp dell'ultima modifica dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `description`: Una descrizione di questo articolo  - `id`: Identificatore unico dell'entità  - `inlim`: Indicatore del limitatore d'ingresso. true = il limitatore d'ingresso Vimax e Vimin è considerato false = il limitatore d'ingresso Vimax e Vimin non è considerato. Valore tipico = true. Valore predefinito: Falso  - `ka`: Guadagno del regolatore di tensione (Ka).  Valore tipico = 1. Predefinito: 0.0  - `kc`: Fattore di carico del raddrizzatore proporzionale alla reattanza di commutazione (Kc). Valore tipico = 0,55. Predefinito: 0.0  - `kd`: Fattore di smagnetizzazione, funzione delle reattanze dell'alternatore di eccitazione (Kd).  Valore tipico = 1,1. Predefinito: 0,0  - `kdr`: Guadagno derivato del regolatore di tensione (Kdr).  Valore tipico = 10. Predefinito: 0.0  - `ke`: Costante di eccitazione relativa al campo autoeccitato (Ke).  Valore tipico = 1. Predefinito: 0,0  - `kir`: Guadagno integrale del regolatore di tensione (Kir).  Valore tipico = 5. Predefinito: 0.0  - `kpr`: Guadagno proporzionale del regolatore di tensione (Kpr).  Valore tipico = 80. Predefinito: 0,0  - `ks`: Coefficiente per permettere un uso diverso del coefficiente di velocità del modello (Ks).  Valore tipico = 0. Predefinito: 0.0  - `location`: Riferimento Geojson all'elemento. Può essere Point, LineString, Polygon, MultiPoint, MultiLineString o MultiPolygon  - `name`: Il nome di questo articolo.  - `owner`: Una lista contenente una sequenza di caratteri codificata in JSON che si riferisce agli ID unici dei proprietari  - `pidlim`: Indicatore del limitatore PID. true = il limitatore di ingresso Vpidmax e Vpidmin è considerato false = il limitatore di ingresso Vpidmax e Vpidmin non è considerato. Valore tipico = true. Valore predefinito: Falso  - `seeAlso`: elenco di uri che puntano a risorse aggiuntive sull'elemento  - `seve1`: Valore della funzione di saturazione dell'eccitatore alla tensione corrispondente dell'eccitatore, Ve, dietro la reattanza di commutazione (Se[Ve1]).  Valore tipico = 0,3. Predefinito: 0,0  - `seve2`: Valore della funzione di saturazione dell'eccitatore alla tensione corrispondente dell'eccitatore, Ve, dietro la reattanza di commutazione (Se[Ve2]).  Valore tipico = 3. Predefinito: 0,0  - `source`: Una sequenza di caratteri che dà la fonte originale dei dati dell'entità come URL. Si raccomanda di essere il nome di dominio completamente qualificato del fornitore di origine, o l'URL dell'oggetto di origine.  - `ta`: Costante di tempo del regolatore di tensione (Ta).  Valore tipico = 0. Predefinito: 0  - `tdr`: Costante di tempo di ritardo (Tdr).  Valore tipico = 0,1. Predefinito: 0  - `te`: Costante di tempo dell'eccitatore, tasso di integrazione associato al controllo dell'eccitatore (Te).  Valore tipico = 1,2. Predefinito: 0  - `telim`: Selettore per il limitatore sul blocco [1/sTe].  Vedere il diagramma per il significato di vero e falso. Valore tipico = falso. Valore predefinito: Falso  - `type`: Tipo NGSI. Deve essere ExcAC8B  - `ve1`: Le tensioni di uscita dell'alternatore di eccitazione dietro la reattanza di commutazione alla quale si definisce la saturazione (Ve) è uguale a V (Ve1).  Valore tipico = 6,5. Predefinito: 0,0  - `ve2`: Tensioni di uscita dell'alternatore di eccitazione dietro la reattanza di commutazione alla quale si definisce la saturazione (Ve).  Valore tipico = 9. Predefinito: 0,0  - `vemin`: Uscita di tensione minima dell'eccitatore (Vemin).  Valore tipico = 0. Predefinito: 0.0  - `vfemax`: Riferimento del limite di corrente di campo dell'eccitatore (Vfemax).  Valore tipico = 6. Predefinito: 0,0  - `vimax`: Segnale d'ingresso massimo (Vimax).  Valore tipico = 35. Predefinito: 0.0  - `vimin`: Segnale di ingresso minimo (Vimin).  Valore tipico = -10. Predefinito: 0.0  - `vpidmax`: Uscita massima del regolatore PID (Vpidmax).  Valore tipico = 35. Predefinito: 0.0  - `vpidmin`: Uscita minima del regolatore PID (Vpidmin).  Valore tipico = -10. Predefinito: 0.0  - `vrmax`: Uscita massima del regolatore di tensione (Vrmax). Valore tipico = 35. Predefinito: 0.0  - `vrmin`: Uscita minima del regolatore di tensione (Vrmin).  Valore tipico = 0. Predefinito: 0.0  - `vtmult`: Indicatore "Multiply by generator`s terminal voltage". true = i limiti Vrmax e Vrmin sono moltiplicati per la tensione terminale del generatore per rappresentare uno stadio di potenza a tiristori alimentato dai terminali del generatore false = i limiti non sono moltiplicati per la tensione terminale del generatore.  Valore tipico = falso. Valore predefinito: Falso    
...
Non disponibile l'esempio di un ExcAC8B in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-v2 quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
...
Non disponibile l'esempio di un ExcAC8B in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-LD quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
