...
Descrizione globale: **Adattato dai modelli di dati CIM. Modello di sistema di eccitazione rotante per scopi generali.  Questo modello può essere usato per rappresentare una vasta gamma di sistemi di eccitazione la cui fonte di alimentazione DC è un generatore AC o DC. Comprende i modelli di sistema di eccitazione di tipo IEEE AC1, AC2, DC1 e DC2.  

## Elenco delle proprietà  

- `address`: L'indirizzo postale  - `alternateName`: Un nome alternativo per questa voce  - `areaServed`: L'area geografica in cui viene fornito un servizio o un articolo offerto  - `dataProvider`: Una sequenza di caratteri che identifica il fornitore dell'entità di dati armonizzata.  - `dateCreated`: Timestamp di creazione dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `dateModified`: Timestamp dell'ultima modifica dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `description`: Una descrizione di questo articolo  - `e1`: Valore di tensione di campo 1 (E1).  Valore tipico = 3. Predefinito: 0,0  - `e2`: Valore di tensione di campo 2 (E2).  Valore tipico = 4. Predefinito: 0,0  - `fbf`: Flag del segnale di feedback del tasso (Fbf). Valore tipico = fieldCurrent. Valore predefinito: Nessuno  - `flimf`: Flag del tipo di limite (Flimf).  Valore tipico = 0. Predefinito: 0.0  - `id`: Identificatore unico dell'entità  - `kc`: Fattore di regolazione del raddrizzatore (Kc).  Valore tipico = 0,05. Predefinito: 0.0  - `kd`: Fattore di regolazione dell'eccitatore (Kd).  Valore tipico = 2. Predefinito: 0,0  - `ke`: Costante proporzionale di campo dell'eccitatore (Ke).  Valore tipico = 1. Predefinito: 0,0  - `kefd`: Guadagno di feedback della tensione di campo (Kefd).  Valore tipico = 0. Predefinito: 0.0  - `kf`: Guadagno di feedback del tasso (Kf).  Valore tipico = 0,05. Predefinito: 0  - `kh`: Guadagno di feedback del regolatore di tensione di campo (Kh).  Valore tipico = 0. Predefinito: 0.0  - `kii`: Guadagno integrale del regolatore di corrente di campo (Kii).  Valore tipico = 0. Predefinito: 0.0  - `kip`: Guadagno proporzionale del regolatore di corrente di campo (Kip).  Valore tipico = 1. Predefinito: 0.0  - `ks`: Coefficiente per permettere un uso diverso del coefficiente di velocità del modello (Ks).  Valore tipico = 0. Predefinito: 0.0  - `kvi`: Guadagno integrale del regolatore di tensione (Kvi).  Valore tipico = 0. Predefinito: 0.0  - `kvp`: Guadagno proporzionale del regolatore di tensione (Kvp).  Valore tipico = 2800. Predefinito: 0.0  - `kvphz`: Guadagno del limitatore V/Hz (Kvphz).  Valore tipico = 0. Predefinito: 0.0  - `location`: Riferimento Geojson all'elemento. Può essere Point, LineString, Polygon, MultiPoint, MultiLineString o MultiPolygon  - `name`: Il nome di questo articolo.  - `nvphz`: Velocità di prelievo del limitatore V/Hz (Nvphz).  Valore tipico = 0. Predefinito: 0.0  - `owner`: Una lista contenente una sequenza di caratteri codificata in JSON che si riferisce agli ID unici dei proprietari  - `se1`: Fattore di saturazione a E1 (Se1).  Valore tipico = 0,0001. Predefinito: 0.0  - `se2`: Fattore di saturazione a E2 (Se2).  Valore tipico = 0,001. Predefinito: 0.0  - `seeAlso`: elenco di uri che puntano a risorse aggiuntive sull'elemento  - `source`: Una sequenza di caratteri che dà la fonte originale dei dati dell'entità come URL. Si raccomanda di essere il nome di dominio completamente qualificato del fornitore di origine, o l'URL dell'oggetto di origine.  - `ta`: Costante di tempo del regolatore di tensione (Ta).  Valore tipico = 0,01. Predefinito: 0  - `tb1`: Costante di tempo di ritardo (Tb1).  Valore tipico = 0. Predefinito: 0  - `tb2`: Costante di tempo di ritardo (Tb2).  Valore tipico = 0. Predefinito: 0  - `tc1`: Costante di tempo (Tc1).  Valore tipico = 0. Predefinito: 0  - `tc2`: Costante di tempo (Tc2).  Valore tipico = 0. Predefinito: 0  - `te`: Costante di tempo del campo dell'eccitatore (Te).  Valore tipico = 1,2. Predefinito: 0  - `tf`: Costante di tempo di feedback del tasso (Tf).  Valore tipico = 1. Predefinito: 0  - `tf1`: Costante di tempo di feedback (Tf1).  Valore tipico = 0. Predefinito: 0  - `tf2`: Costante di tempo di ritardo della retroazione (Tf2).  Valore tipico = 0. Predefinito: 0  - `tp`: Costante di tempo del ponte della corrente di campo (Tp).  Valore tipico = 0. Predefinito: 0  - `type`: Tipo NGSI. Deve essere ExcREXS  - `vcmax`: Tensione massima di compounding (Vcmax).  Valore tipico = 0. Predefinito: 0.0  - `vfmax`: Massima corrente di campo dell'eccitatore (Vfmax).  Valore tipico = 47. Predefinito: 0.0  - `vfmin`: Corrente di campo minima dell'eccitatore (Vfmin).  Valore tipico = -20. Predefinito: 0.0  - `vimax`: Limite di ingresso del regolatore di tensione (Vimax).  Valore tipico = 0.1. Predefinito: 0.0  - `vrmax`: Uscita massima del regolatore (Vrmax).  Valore tipico = 47. Predefinito: 0,0  - `vrmin`: Uscita minima del regolatore (Vrmin).  Valore tipico = -20. Predefinito: 0.0  - `xc`: Reattanza composta dell'eccitatore (Xc).  Valore tipico = 0. Predefinito: 0.0    
...
Non disponibile l'esempio di un ExcREXS in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-v2 quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
...
Non disponibile l'esempio di un ExcREXS in formato JSON-LD come normalizzato. Questo è compatibile con NGSI-LD quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
