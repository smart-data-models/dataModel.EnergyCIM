...
Description globale : **Adaptée des modèles de données CIM. Il s'agit d'une classe racine destinée à fournir une identification commune à toutes les classes qui ont besoin d'attributs d'identification et de dénomination**.  
...
Non disponible l'exemple d'un IdentifiedObject au format JSON-LD comme key-values. Ceci est compatible avec NGSI-v2 lorsque l'on utilise `options=keyValues` et renvoie les données contextuelles d'une entité individuelle.  
Non disponible l'exemple d'un IdentifiedObject au format JSON-LD tel que normalisé. Ceci est compatible avec la NGSI-v2 lorsqu'elle n'utilise pas d'options et renvoie les données contextuelles d'une entité individuelle.  
Non disponible l'exemple d'un IdentifiedObject au format JSON-LD comme key-values. Ceci est compatible avec NGSI-LD lorsque l'on utilise `options=keyValues` et renvoie les données contextuelles d'une entité individuelle.  
