...
グローバルな記述です。**CIMのデータモデルからの採用です。このクラスはIEEE Std 421.5-2005のタイプST4Bモデルを表す。このモデルはタイプST3Aモデルのバリエーションで，ST3Aモデルにあるラグ・リードレギュレータ特性に代わって比例＋積分（PI）レギュレータブロックが搭載されている。電位整流器と複合電源整流器の両方の励磁システムをモデル化しています。  PIレギュレータブロックは、非巻上げ限界を表現しています。このモデルの電圧レギュレータは、通常、デジタルで実装されます。  参考文献IEEE規格421.5-2005 7.4.**項  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `kc`: 整流器の負荷率が整流リアクタンス（K）に比例すること。代表値＝0.113。初期値：0.0  - `kg`: インナーループ・フィールド・レギュレーター（K）のフィードバックゲイン定数。  代表値＝0、初期値＝0.0  - `ki`: 電位回路利得係数（K）。  代表値＝0、初期値＝0.0  - `kim`: 電圧調整器の積分ゲイン出力（K）。  代表値＝0、初期値＝0.0  - `kir`: 電圧調整器の積分利得（K）。  代表値＝10.75初期値：0.0  - `kp`: 電位回路の利得係数（K）。  代表値＝9.3。初期値：0.0  - `kpm`: 電圧調整器の比例ゲイン出力（K）。  代表値＝1。初期値：0.0  - `kpr`: 電圧調整器の比例ゲイン（K）。  代表値＝10.75。初期値：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `ta`: 電圧調整器の時定数（T）。  代表値＝0.02。デフォルト：0  - `thetap`: 電位回路の位相角（thetap）。  代表値＝0、デフォルト：0.0  - `type`: NGSIタイプ。ExcIEEEST4Bである必要があります。  - `vbmax`: 最大励磁電圧（V）。  代表値＝11.63初期値：0.0  - `vmmax`: インナーループの最大出力（V）。  代表値＝99。デフォルト：0.0  - `vmmin`: インナーループの最小出力（V）。  代表値＝-99。初期値：0.0  - `vrmax`: ボルテージレギュレータの最大出力（V）。  代表値＝1。初期値：0.0  - `vrmin`: ボルテージレギュレーター出力の最小値（V）。  代表値＝-0.87。初期値：0.0  - `xl`: 電位源（X）に関連するリアクタンス。  代表値＝0.124。初期値：0.0    
...
JSON-LD形式のExcIEEEST4Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST4Bの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcIEEEST4Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST4Bの例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
