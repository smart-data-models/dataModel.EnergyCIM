...
グローバルな記述です。**CIMデータモデルからの適応。送電線や配電線の電圧レベルでのエネルギー供給者のための一般的な等価物である。  

## プロパティのリスト  

- `EnergySchedulingType`: 特定のエネルギー・スケジューリング・タイプのエネルギー源 デフォルトなし  - `WindTurbineType3or4Dynamics`: 風力発電機 このエネルギー源に関連するタイプ3または4のダイナミクスモデル。デフォルトはなし  - `activePower`: 高電圧源アクティブインジェクション。正の符号はノードからの流出を意味し、負荷符号規則を使用する。定常解の開始値。デフォルト：0.0  - `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `nominalVoltage`: 位相間の公称電圧。デフォルト：0.0  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `r`: 正の配列のテベニン耐性。デフォルト：0.0  - `r0`: ゼロ・シーケンス・テベニン抵抗。デフォルト：0.0  - `reactivePower`: 高電圧源の無効注入。正の符号はノードからの流出を意味する、負荷符号規則を使用する。定常解の開始値。デフォルト：0.0  - `rn`: 負の配列のテベニン耐性。デフォルト：0.0  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `type`: NGSIタイプです。EnergySourceでなければなりません。  - `voltageAngle`: a-phase open circuitの位相角。デフォルト：0.0  - `voltageMagnitude`: 位相間開放電圧の大きさ。デフォルト：0.0  - `x`: 正系列のテベニンリアクタンス。デフォルト：0.0  - `x0`: Zero sequence Thevenin reactance.デフォルト：0.0  - `xn`: 負のシーケンス・テベニンリアクタンス。デフォルト：0.0    
...
JSON-LD形式のEnergySourceの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のEnergySourceの例を正規化したものはありません。これはオプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のEnergySourceの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のEnergySourceの例を正規化したものはありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
