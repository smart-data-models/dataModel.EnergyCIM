...
グローバルな記述です。**CIMのデータモデルからの採用です。このクラスはIEEE Std 421.5-2005のタイプAC4Aモデルを表しています。このモデルは、他のタイプのACシステムとは全く異なる、AC4Aタイプのオルタネータ供給型制御整流器励起システムを表しています。この初期応答性の高い励磁システムは、励磁出力回路にフルサイリスタブリッジを採用しています。  電圧調整器がサイリスタブリッジの点火を制御します。励磁機のオルタネーターは、独立した電圧レギュレータを使用して、その出力電圧を一定に制御します。これらの効果はモデル化されていませんが、エキサイタオルタネータの過渡負荷効果は含まれています。   参考文献IEEE規格421.5-2005 6.4.**項  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `ka`: 電圧調整器のゲイン（K）。  代表値＝200。初期値：0.0  - `kc`: 整流器の負荷係数で，整流リアクタンス（K）に比例する。  代表値＝0、デフォルト：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `ta`: 電圧調整器の時定数（T）。  代表値＝0.015。初期値：0  - `tb`: 電圧調整器の時定数（T）。  代表値＝10。デフォルト：0  - `tc`: 電圧調整器の時定数（T）。  代表値＝1。デフォルト：0  - `type`: NGSIタイプです。ExcIEEEAC4Aである必要があります。  - `vimax`: ボルテージレギュレーターの最大入力制限値（V）。  代表値＝10。初期値：0.0  - `vimin`: 電圧調整器の入力制限の下限値（V）。  代表値＝-10。初期値：0.0  - `vrmax`: ボルテージレギュレーターの最大出力（V）。  代表値＝5.64。初期値：0.0  - `vrmin`: ボルテージレギュレーターの最低出力（V）。  代表値＝-4.53。初期値：0.0    
...
JSON-LD形式のExcIEEEAC4Aの例をkey-valuesとして利用できません。これは`options=keyValues`を使った場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEAC4Aの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcIEEEAC4Aの例をkey-valuesとして利用できません。これは`options=keyValues`を使った場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEAC4Aの例を利用できません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
