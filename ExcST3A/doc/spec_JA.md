...
グローバルな記述です。**CIMデータモデルから採用。IEEE ST3A静電容量方式に回転数を追加したもの。  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `efdmax`: AVRの最大出力（Efdmax）。  代表値＝6.9です。初期値：0.0  - `id`: エンティティのユニークな識別子  - `kc`: 整流器の負荷率が整流リアクタンス（Kc）に比例すること。代表値＝1.1。初期値：0.0  - `kg`: インナーループ・フィールドレギュレーターのフィードバックゲイン定数（Kg）。  代表値＝1。デフォルト：0.0  - `ki`: 電位回路の利得係数（Ki）。  代表値＝4.83。初期値：0.0  - `kj`: AVRゲイン（Kj）。  代表値＝200。デフォルト：0.0  - `km`: インナーループフィールドレギュレーターのフォワードゲイン定数（Km）。  代表値＝7.04。デフォルト：0.0  - `kp`: 潜在的なソースゲイン（Kp）（＞0）。  代表値＝4.37。デフォルト：0.0  - `ks`: モデルスピード係数(Ks)を使い分けるための係数。  代表値＝0、初期値＝0.0  - `ks1`: モデルスピード係数(Ks1)を使い分けるための係数。  代表値＝0、初期値＝0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `tb`: 電圧調整器の時定数（Tb）。  代表値＝6.67です。デフォルト：0  - `tc`: 電圧調整器の時定数（Tc）。  代表値＝1。デフォルト：0  - `thetap`: 電位回路の位相角（ザータップ）。  代表値＝20。デフォルト：0.0  - `tm`: インナーループ・フィールド・レギュレーターの順方向時定数（Tm）。  代表値＝1。デフォルト：0  - `type`: NGSIタイプです。ExcST3Aである必要があります。  - `vbmax`: 最大励磁電圧（Vbmax）。  代表値＝8.63初期値：0.0  - `vgmax`: 最大インナーループフィードバック電圧（Vgmax）。  代表値＝6.53初期値：0.0  - `vimax`: ボルテージレギュレーターの最大入力制限（Vimax）。  代表値＝0.2。初期値：0.0  - `vimin`: 電圧調整器の入力制限の最小値（Vimin）。  代表値＝-0.2初期値：0.0  - `vrmax`: ボルテージレギュレータの最大出力（Vrmax）。  代表値＝1。初期値：0.0  - `vrmin`: ボルテージレギュレーターの最小出力（Vrmin）。  代表値＝0、デフォルト：0.0  - `xl`: 電位源(Xl)に関連するリアクタンス。  代表値＝0.09。初期値：0.0    
...
JSON-LD形式のExcST3Aの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcST3Aの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
ExcST3Aの例をkey-valuesとしてJSON-LD形式で利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcST3Aの例を利用できません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
