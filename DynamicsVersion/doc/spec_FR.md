...
Non disponible l'exemple d'une version dynamique au format JSON comme valeurs clés. Cette version est compatible avec NGSI V2 lorsqu'elle utilise "options=keyValues" et renvoie les données de contexte d'une entité individuelle.  
Non disponible l'exemple d'une version dynamique au format JSON comme normalisé. Cette version est compatible avec NGSI V2 lorsqu'elle n'utilise pas d'options et renvoie les données de contexte d'une entité individuelle.  
Non disponible l'exemple d'une version dynamique au format JSON-LD comme valeurs clés. Cette version est compatible avec le format NGSI-LD lorsqu'elle utilise "options=keyValues" et renvoie les données de contexte d'une entité individuelle.  
Non disponible l'exemple d'une version de Dynamics en format JSON-LD comme normalisé. Cette version est compatible avec le format JSON-LD lorsqu'elle n'utilise pas d'options et renvoie les données de contexte d'une entité individuelle.  
