...
グローバルな記述です。**CIMのデータモデルからの採用です。このクラスは，IEEE Std 421.5-2005タイプのST6Bモデルを表す。このモデルは、内部ループのフィールド電圧レギュレータとプリコントロールを備えたPI電圧レギュレータで構成されています。フィールド電圧レギュレータは比例制御を行います。プリコントロールとフィードバック回路の遅延により，動的応答が向上しています．  参考文献IEEE規格 421.5-2005 7.6.**項  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `ilr`: エキサイター出力電流制限値の目安（I）。  代表値＝4.164初期値：0.0  - `kci`: エキサイター出力電流リミット調整（K）。  代表値＝1.0577。初期値：0.0  - `kff`: インナーループフィールドレギュレータ（K）のプリコントロールゲイン定数。代表値＝1。デフォルト：0.0  - `kg`: インナーループフィールドレギュレーター（K）のフィードバックゲイン定数。  代表値＝1。初期値：0.0  - `kia`: 電圧調整器の積分利得（K）。  代表値＝45.094初期値：0.0  - `klr`: エキサイター出力電流リミッターゲイン（K）。  代表値＝17.33初期値：0.0  - `km`: インナーループ・フィールド・レギュレーター（K）のフォワードゲイン定数。  典型的な値は1です。デフォルト：0.0  - `kpa`: ボルテージレギュレータの比例ゲイン（K）。  代表値＝18.038。初期値：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `oelin`: OEL入力セレクタ（OELin）。代表値＝noOELinput。デフォルトなし  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `tg`: インナーループフィールドボルテージレギュレーター（T）のフィードバック時定数。代表値＝0.02。デフォルト：0  - `type`: NGSIタイプ。ExcIEEEST6Bである必要があります。  - `vamax`: ボルテージレギュレーターの最大出力（V）。  代表値＝4.81初期値：0.0  - `vamin`: ボルテージレギュレータの最低出力電圧 (V).  代表値＝-3.85初期値：0.0  - `vrmax`: ボルテージレギュレーターの最大出力（V）。  代表値＝4.81初期値：0.0  - `vrmin`: ボルテージレギュレータの最低出力電圧 (V).  代表値＝-3.85初期値：0.0    
...
JSON-LD形式のExcIEEEST6Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST6Bの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcIEEEST6Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST6Bの例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
