...
Globale Beschreibung: **Abgeleitet aus CIM-Datenmodellen. Ein Draht oder eine Kombination von Drähten mit konsistenten elektrischen Eigenschaften, die ein einzelnes elektrisches System bilden und zur Übertragung von Wechselstrom zwischen Punkten im Stromnetz verwendet werden. Für symmetrische, transponierte 3ph-Leitungen genügt es, Attribute des Leitungssegments zu verwenden, die Impedanzen und Admittanzen für die gesamte Länge des Segments beschreiben.  Zusätzlich können die Impedanzen über die Länge und die zugehörigen Impedanzen pro Länge berechnet werden. Die BaseVoltage an den beiden Enden von ACLineSegments in einer Leitung muss die gleiche BaseVoltage.nominalVoltage haben. Grenzlinien können jedoch geringfügig unterschiedliche BaseVoltage.nominalVoltage haben und eine Variation ist zulässig. Ein größerer Spannungsunterschied erfordert im Allgemeinen die Verwendung eines äquivalenten Abzweigs.**  
...
Nicht verfügbar ist das Beispiel eines ACLineSegments im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-v2 bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
Nicht verfügbar das Beispiel eines ACLineSegments im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden und liefert die Kontextdaten einer einzelnen Entität.  
Nicht verfügbar das Beispiel eines ACLineSegments im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-LD bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
Nicht verfügbar das Beispiel eines ACLineSegments im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden und liefert die Kontextdaten einer einzelnen Entität.  
