...
グローバルな記述です。**CIMデータモデルから採用。IEEE AC5Aオルタネータ供給整流器励磁システムの最小コントローラ出力を変更したもの **。  

## プロパティのリスト  

- `a`: モデル(a)の異なる使い方を可能にする係数。  典型的な値は1です。既定値：0.0  - `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `efd1`: エキサイター飽和が定義されるエキサイター電圧（Efd1）。  代表値＝5.6です。初期値：0.0  - `efd2`: エキサイター飽和が定義されるエキサイター電圧（Efd2）。  代表値＝4.2です。初期値：0.0  - `id`: エンティティのユニークな識別子  - `ka`: 電圧調整器のゲイン（Ka）。  代表値＝400。初期値：0.0  - `ke`: 自励磁電界（Ke）に関係するエキサイター定数。  典型的な値は1です。初期値：0.0  - `kf`: 励磁制御系スタビライザーゲイン（Kf）。  代表値＝0.03。初期値：0.0  - `ks`: モデルスピード係数(Ks)を使い分けるための係数。  代表値＝0、初期値＝0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `seefd1`: 対応するエキサイタ電圧Efd1におけるエキサイタ飽和関数値（S[Efd1]）。  代表値＝0.86。初期値：0.0  - `seefd2`: 対応するエキサイタ電圧Efd2におけるエキサイタ飽和関数値（S[Efd2]）。  代表値＝0.5。初期値：0.0  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `ta`: 電圧調整器の時定数（Ta）。  代表値＝0.02。デフォルト：0  - `tb`: 電圧調整器の時定数（Tb）。  代表値＝0 既定値：0  - `tc`: 電圧調整器の時定数（Tc）。  代表値＝0、デフォルト：0  - `te`: エキサイター時定数、エキサイター制御(Te)に伴う積分率。  代表値＝0.8。初期値：0  - `tf1`: 励磁制御系安定化時定数（Tf1）。  代表値＝1。デフォルト：0  - `tf2`: 励磁制御系安定化時定数（Tf2）。  代表値＝0.8。デフォルト：0  - `tf3`: 励磁制御系安定化時定数（Tf3）。  代表値＝0、初期値＝0  - `type`: NGSIタイプです。ExcAC5Aである必要があります。  - `vrmax`: ボルテージレギュレーターの最大出力（Vrmax）。  代表値＝7.3。初期値：0.0  - `vrmin`: ボルテージレギュレーターの最小出力（Vrmin）。  代表値＝7.3初期値：0.0    
...
JSON-LD形式のExcAC5Aの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcAC5Aの例を利用できません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcAC5Aの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcAC5Aの例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
