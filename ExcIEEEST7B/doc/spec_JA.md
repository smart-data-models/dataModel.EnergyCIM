...
グローバルな記述です。**CIMのデータモデルからの採用です。このクラスはIEEE Std 421.5-2005タイプのST7Bモデルを表しています。このモデルは静的な電位源の励磁システムを代表するものです。このシステムでは、AVRはPI電圧調整器で構成されています。位相リードラグフィルタを直列に配置することで、ブラシレス励磁システムで一般的に使用される微分関数を導入することができます。この場合、レギュレータはPIDタイプになります。さらに、端子電圧チャンネルには、位相リードラグフィルターが含まれています。  AVRは、過励磁リミッター（OEL1）、不足励磁リミッター（UEL）、固定子電流リミッター（SCL）、電流補償器（DROOP）のための適切な入力を基準に含んでいます。これらの制限はすべて、電圧リファレンスレベルで動作している場合、PSS（Type PSS1A、PSS2A、PSS2BからのVS信号）の動作を維持します。しかし、UELの制限は、出力信号に作用するハイバリュー（HV）ゲートにも伝わります。さらに、出力信号は天井過昇リミッター（OEL2）用の低値（LV）ゲートを通過します。  参考にしてください。IEEE規格421.5-2005 7.7.**項  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `kh`: 高価なゲートフィードバックゲイン（K）。  代表値 1.初期値：0.0  - `kia`: 電圧調整器の積分ゲイン（K）。  典型的な値は1です。初期値：0.0  - `kl`: 低い値のゲートフィードバックゲイン（K）。  代表値 1.初期値：0.0  - `kpa`: 電圧調整器の比例ゲイン（K）。  代表値＝40。初期値：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `oelin`: OEL入力セレクタ（OELin）。代表値＝noOELinput。デフォルトなし  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `tb`: レギュレータ・ラグ時定数（T）。  代表値 1.デフォルト：0  - `tc`: レギュレータリード時定数（T）。  代表値 1.デフォルト：0  - `tf`: 励磁制御系安定化時定数（T）。  代表値 1.初期値：0  - `tg`: インナーループフィールドボルテージレギュレーターのフィードバック時定数（T）。代表値 1初期値：0  - `tia`: フィードバック時定数（T）。  代表値＝3デフォルト：0  - `type`: NGSIタイプ。ExcIEEEST7Bである必要があります。  - `uelin`: UEL入力セレクタ（UELin）。代表値＝noUELinput。デフォルトなし  - `vmax`: 最大電圧基準信号（V）。  代表値＝1.1。初期値：0.0  - `vmin`: 最小電圧基準信号（V）。  代表値＝0.9初期値：0.0  - `vrmax`: ボルテージレギュレータの最大出力（V）。  代表値＝5。初期値：0.0  - `vrmin`: ボルテージレギュレーターの最低出力（V）。  代表値＝-4.5。初期値：0.0    
...
JSON-LD形式のExcIEEEST7Bの例をkey-valuesとして利用できません。これは、`options=keyValues`を使用した場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST7Bの例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
JSON-LD形式のExcIEEEST7Bの例をkey-valuesとして利用できません。これは`options=keyValues`を使った場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
正規化されたJSON-LD形式のExcIEEEST7Bの例を利用できません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
