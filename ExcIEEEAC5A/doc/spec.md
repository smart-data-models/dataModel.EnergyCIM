...
#### ExcIEEEAC5A NGSI V2 key-values Example    
Here is an example of a ExcIEEEAC5A in JSON format as key-values. This is compatible with NGSI V2 when  using `options=keyValues` and returns the context data of an individual entity.  
#### ExcIEEEAC5A NGSI V2 normalized Example    
Here is an example of a ExcIEEEAC5A in JSON format as normalized. This is compatible with NGSI V2 when not using options and returns the context data of an individual entity.  
#### ExcIEEEAC5A NGSI-LD key-values Example    
Here is an example of a ExcIEEEAC5A in JSON-LD format as key-values. This is compatible with NGSI-LD when  using `options=keyValues` and returns the context data of an individual entity.  
#### ExcIEEEAC5A NGSI-LD normalized Example    
Here is an example of a ExcIEEEAC5A in JSON-LD format as normalized. This is compatible with NGSI-LD when not using options and returns the context data of an individual entity.  
