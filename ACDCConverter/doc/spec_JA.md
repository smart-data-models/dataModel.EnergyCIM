...
グローバルな記述です。**CIMデータモデルから採用。三相用のバルブを備えたユニットで，ユニット制御装置，必須の保護装置及びスイッチング装置，DC貯蔵コンデンサ，相リアクトル，及び変換に使用される補助装置（ある場合）を含む。  

## プロパティのリスト  

- `DCTerminals`: デフォルト：'list'  - `PccTerminal`: すべてのコンバータのDC側は、この共通結合端子のポイントにリンクされています。デフォルトなし  - `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `baseS`: コンバーターポールの基本皮相電力デフォルト：0.0  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `id`: エンティティのユニークな識別子  - `idc`: コンバータの直流電流。Idとも呼ばれる。コンバータの状態変数で、パワーフローから得られる。デフォルト：0.0  - `idleLoss`: 無給電時の極の有効電力損失。パワーフローで使用されるコンバータ構成データ。デフォルト：0.0  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `maxUdc`: コンバータが動作すべきDC側の最大電圧。パワーフローで使用されるコンバーターの設定データ。デフォルト：0.0  - `minUdc`: 最小許容コンバータDC電圧。パワーフローで使用されるコンバータの設定データ。デフォルト：0.0  - `name`: このアイテムの名前です。  - `numberOfValves`: コンバーターのバルブの数。損失計算に使用される。デフォルト：0  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `p`: 共通結合点における有効電力。正の符号はノードからの流出を意味するなど、負荷の符号規則が使用される。簡略化されたパワーフローモデルが使用されている場合の、定常状態ソリューションのための開始値。デフォルト：0.0  - `poleLossP`: DCポールにおける有効電力損失 = idleLoss + switchingLoss*|Idc| + resitiveLoss*Idc^2 ロスレス動作の場合 Pdc=Pac 損失を伴う整流器動作の場合 Pdc=Pac-lossP 損失を伴うインバータ動作の場合 Pdc=Pac+lossP パワーフローで使用されるコンバータ状態変数。デフォルト：0.0  - `q`: 共通結合点におけるリアクティブパワー。正の符号はノードからの流出を意味するなど、負荷の符号規則が使用されている。簡略化されたパワーフローモデルが使用されている場合の、定常状態ソリューションのための開始値。デフォルト：0.0  - `ratedUdc`: コンバータの定格DC電圧。UdNとも呼ばれる。パワーフローで使用されるコンバータ構成データ。デフォルト：0.0  - `resistiveLoss`: パワーフローで使用されるコンバータの設定データです。poleLossP」を参照してください。デフォルト：0.0  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `switchingLoss`: 基本皮相電力`baseS`に対するスイッチング損失。poleLossPを参照。デフォルト：0.0  - `targetPpcc`: 共通結合点におけるACグリッドへの実電力注入目標値。デフォルト：0.0  - `targetUdc`: DC電圧の大きさの目標値デフォルト：0.0  - `type`: NGSIタイプであること。ACDCConverterである必要があります。  - `uc`: コンバータ電圧（ブリッジのAC側の電圧）。コンバータの状態変数、パワーフローの結果。デフォルト：0.0  - `udc`: DC側のコンバータ電圧。Ud.コンバータの状態変数とも呼ばれ、パワーフローから得られる。デフォルト：0.0  - `valveU0`: バルブのしきい値電圧。バルブが導通しているときの順方向の電圧降下。switchLossはnumberOfValves * valveU0に依存します。デフォルト：0.0    
...
ACDCConverterの例をkey-valuesとしてJSON-LD形式にしたものは利用できません。これは`options=keyValues`を使うとNGSI-v2と互換性があり、個々のエンティティのコンテキストデータを返します。  
ACDCConverterをJSON-LD形式で正規化した例はありません。オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
ACDCConverterの例をkey-valuesとしてJSON-LD形式にしたものは利用できません。これは`options=keyValues`を使うとNGSI-LDと互換性があり、個々のエンティティのコンテキストデータを返します。  
ACDCConverterをJSON-LD形式で正規化した例はありません。オプションを使用しない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
