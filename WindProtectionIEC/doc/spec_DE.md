...
Globale Beschreibung: **Abgeleitet aus CIM-Datenmodellen. Das Netzschutzmodell umfasst den Schutz gegen Über- und Unterspannung sowie gegen Über- und Unterfrequenz.  Referenz: IEC-Norm 614000-27-1 Abschnitt 6.6.6.**  
...
Nicht verfügbar das Beispiel einer WindProtectionIEC im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-v2 bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
Nicht verfügbar das Beispiel einer WindProtectionIEC im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-v2, wenn keine Optionen verwendet werden und liefert die Kontextdaten einer einzelnen Entität.  
Nicht verfügbar das Beispiel einer WindProtectionIEC im JSON-LD-Format als Key-Values. Dies ist kompatibel mit NGSI-LD bei Verwendung von `options=keyValues` und liefert die Kontextdaten einer einzelnen Entität.  
Nicht verfügbar das Beispiel einer WindProtectionIEC im JSON-LD-Format als normalisiert. Dies ist kompatibel mit NGSI-LD, wenn keine Optionen verwendet werden und liefert die Kontextdaten einer einzelnen Entität.  
